			...
			new ReadableStream({
				...
				async write(data) {
					...
					const connectStreams = async (remoteSocket, webSocket, headerData, retryFunc) => {
						...
						const timeoutId = setTimeout(() => { if (!hasData) dataPromiseResolve(false); }, userConfig.fallbackTimeout);
					...
					const connectParallel = async () => {
						const tryConnect = async (type) => {
							try {
								...
								if (type === 'proxy') {
									if (PROXY_IP) { const [ph, pp = port] = PROXY_IP.split(':'); return await connectDirect(ph, +pp || port, payload); }
								}
							} catch {}
							return null;
						};
